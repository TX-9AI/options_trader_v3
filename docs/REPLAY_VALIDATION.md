...
**v1.0 — 2026-07-11.** Companion to `tests/replay_confluence.py` v1.0 and
...
1-min OHLC** (`data/OHLC/<date>/<SYMBOL>.csv`), **not** the live yfinance shadow
observer. This is deliberate and load-bearing: the ROADMAP warns against calibrating
on a divergent feed ("a board the bot never plays on"). The DXFeed CSVs are the
traded tape the fills price against; the observer scores off yfinance. So for
regime-truth calibration the CSVs are the correct substrate, and they already
accumulate fleet-wide on every box — no fleet observer deployment required.
