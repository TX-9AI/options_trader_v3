...
- **v4.31 — 2026-08-13 — WH.1b: THE WAREHOUSE LAYOUT IS SPECIFIED (`docs/WAREHOUSE_LAYOUT.md` v1.0).**
  ...
  the conventions were being extended one stream at a time rather than designed.
  Operator: *"we didn't really discuss a file system plan at length — we kind of
  just agreed on the format but not in depth what the store should look like."*
  - **THREE DOMAINS SPLIT BY WHO OWNS THE TRUTH.** `raw/` (box facts,
    never rebuildable, never deleted), `derived/` (control analysis, free to
    delete), `meta/` (fleet context + schema registry). This is the structural
    fix for `reports/` holding analysis products that are also the only copy of
    themselves.
  - **THE JOURNAL ALREADY FINGERPRINTS ITS OWN CODE VERSION AND NOBODY HAD
    CATALOGUED IT.** Every signal-journal line carries a `ruleset` hash. Most
    boxes show `0d70673`; TLT `97864a4` and XOM `45c2f78` because their latest
    journals predate a deploy. `schema_version` = shape of the stored object,
    `ruleset` = which logic decided it. Pooling journal events across a deploy
    boundary without grouping by `ruleset` blends incompatible decision logic.
  - **NO NAIVE-LOCAL TIMESTAMPS EXIST ANYWHERE IN THE SYSTEM.** Measured 29/29:
    trades are ISO `+00:00`; chains/journal/shadow/OHLC are ISO with explicit
    `-04:00`; feed_store is epoch-ms; eod is an ET date string. So the operator's
    UTC-storage rule is mechanically exact and the November DST hazard — a
    repeated wall-clock hour, unrecoverable after the fact — does not apply.
  - **ORB STATE HAS NO LOG AND IS REWRITTEN EVERY TICK.** `open(path,"w")` per
    tick, so every historical ORB state the fleet ever produced is already gone.
    Captured on STATE (`ESTABLISHED`) rather than on clock time, so no window is
    missed and each `attempt` survives separately.
  - **EOD P&L RETAINS ONE DAY.** `pnl_today.json` has no date in its filename and
    is overwritten per session; SMCI still holds 2026-07-15. Silent loss.
  - **OPERATOR DECISIONS RECORDED:** feed_store retains all intervals; SPX is the
    sole VIX writer (safe because **SPX and QQQ trade every day without
    exception**, which also makes them the anchors for any once-per-fleet job);
    `bot.log` is NOT warehoused; `raw/` never expires and `derived/` ages.
  - **A JUSTIFICATION WITHDRAWN.** The scrub was partly sold on SPX sitting at
    91% root vs the fleet's 75%. It does not hold: SPX's composition is identical
    to every other box and the gap is a 2 GB `/swapfile` the others lack, almost
    certainly a deliberate OOM mitigation. `DF_USED=6110` vs `DU_ROOT=6109`,
    `DELOPEN=0` — nothing hidden, nothing to clean. The warehouse stands on
    durability and centralization, not disk relief.
  - **STILL OPEN:** feed_store/OHLC overlap; whether `trades.entry_snapshot`
    already carries chain state at entry (changes whether the chain-trade join is
    built or merely verified); OHLC `volume` carrying decimals; cost ceiling;
    `derived/` and `meta/` specified but unbuilt.
  - **⚠️ NO REPORT READS THE WAREHOUSE YET.** 40 and 41 remain the named
    benchmarks and the 07-23 chain-archive trap is still open.

- **v4.30 — 2026-08-12 — WH.1: THE WAREHOUSE PUSH PATH IS LIVE ON ALL 29 BOXES.**
  Bucket `vertigo-warehouse-tx9ai` (us-east-2, versioned, all public access
  blocked, SSE-S3) created; policy `VertigoWarehouseBoxWrite` (Put/Get on
  objects, List on the bucket, **no Delete anywhere**) attached to a new role
  `day-trader-box`, attached in turn to all 29 bot instances. 1-REPORTER keeps
  `day-trader-control` and was deliberately excluded.
  - **THE HANDOFF WAS WRONG ABOUT CREDENTIALS AND IT WAS WORTH MEASURING.**
    S3_HANDOFF_CONTEXT §5 states the boxes already carry an IAM role. They did
    not carry one at all: `NoCredentialsError` on 15/15 through option 14 and
    again on a direct session. The boxes were only ever TAGGED so control could
    find them; control held every permission. Box push therefore required a
    capability the fleet had never had, which nobody had scoped.
  - **BOTO3 IS INTERPRETER-DEPENDENT ON A BOX, WHICH A NAIVE CHECK HIDES.**
    System `/usr/bin/python3` has boto3 1.40.72 on 15/15; the bot venv
    (`~/options-trader/venv/bin/python`, what `setup_ec2.sh:284` runs) has none.
    The fan-out is non-interactive so it never sources the `.bashrc` activate
    line — the first check looked like a clean pass and was answering for the
    wrong interpreter. Nothing in this repo installs boto3 and
    `requirements.txt` does not declare it: the same undeclared-dependency
    class as the `requests` incident already recorded in that file.
  - **THAT SPLIT IS WHY THIS IS FREEZE-SAFE.** The pusher runs as its own
    systemd timer under system python, not in the trading loop and not in the
    venv. No venv change, no bot restart, no reboot, no execution-path edit —
    so it does not wait for Mon Aug 17. Cost: it may import stdlib + boto3
    only. sqlite3/gzip/json are stdlib, so the whole inventory stays reachable.
  - **CADENCE IS SET BY WHEN BOXES GO DARK, NOT BY CONVENIENCE.** Chains are
    written only by boxes that traded, and those boxes are STOPPED after the
    session; selection picks movers, so one may not wake for weeks, and its
    archive is unreachable that whole time and gone if the box is rebuilt.
    Chains are the one stream nothing can regenerate. Hence a 5-minute push
    plus an `OnBootSec` catch-up, never an EOD push — the same trap already
    documented in `install_candle_logger_timer.sh`, whose 16:05 timer never
    fires if the sweep stops boxes at 15:55.
  - **KEY CONVENTION, and a deliberate deviation from handoff decision #2.**
    `raw/chain_snapshots/dt=<date>/sym=<SYM>/<epoch_ms>-<sha256[:16]>.json`.
    Hive-style so Athena/Glue discover partitions without a parser. The suffix
    is a CONTENT HASH, not the specified uuid4: a uuid makes every retry write
    a duplicate object, a content hash makes the push idempotent. Proven — a
    deliberately erased ledger re-pushes and produces zero duplicates.
    `schema_version` is stamped in every object body, never inferred from key.
  - **CONFIRMED MEANS READ-BACK-AND-COMPARE.** Every object is re-read and
    byte-compared before the ledger advances; the ledger is what a future
    scrub will gate deletion on, so a false confirmation there is data loss,
    not a reporting error. Deliberate-failure proven: a stub that tampers on
    read leaves 0 confirmed and the ledger untouched **after a successful
    PUT** — which is what makes it a test of the verify step rather than of
    the put.
  - `tests/test_s3_push.py` v1.0 — **30 checks, all passing**, including
    resume, append-only advance, partial-tail safety, stale-offset restart,
    rejected put, tampered read-back, and idle-box silence.
  - **⚠️ NOT YET BAKED, AND NOT YET RUN AGAINST REAL S3.** Built and proven on
    the desk against a stub. BUILT, not PUSHED, not BAKED. Nothing about
    today's data changes until the timer is installed on the fleet.
  - **STILL OPEN:** the `canary/` objects from the credential proof need
    deleting (scaffolding must not survive install); control still has no read
    policy on the bucket; trades/journal/feed_store streams are not migrated;
    no report yet reads from the warehouse, which is the 07-23 chain-archive
    trap this build must not repeat.
