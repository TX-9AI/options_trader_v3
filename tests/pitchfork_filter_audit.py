#!/usr/bin/env python3
"""
tests/pitchfork_filter_audit.py — v1.0 — 2026-08-01
...
def main(argv) -> int:
    ...
    elif length_side > filter_side:
        print(f"VERDICT  CORPUS LENGTH ({length_side} vs {filter_side}). The series "
              f"does not yet\n         contain qualifying structure. PF.2 is blocked "
              f"on TIME, not design —\n         bars accrue ~7/session. Do NOT loosen "
              f"the §4.3 priors to force forks.")
    else:
